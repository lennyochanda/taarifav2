# Taarifa Digital Website

This is a [SvelteKit](https://kit.svelte.dev/) and MarkDown powered website built on top of [sveltekit-static-blog-starter](https://www.github.com/josh-collinsworth/sveltekit-blog-starter). It contains the following:
...
Clone or download [this repo](https://github.com/dak-384/taarifav2), then inside of it, in the terminal, run:
...
That should get a dev server up and running (assuming you have npm and Node installed already) and open your default browser. Any saved changes to components and styles should auto-refresh blazingly fast.


## Adding new posts

Adding new posts is as simple as dropping a new `.md` file into `src/routes`. 
To add a new post to the Entertainment page, run (on the terminal):

```
cd src/routes/entertainment/posts
touch new-post-title.md
```
You can then edit the `.md` file. The newly added post will automatically show up on the site, and be added to the posts API.
